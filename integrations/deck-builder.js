...
async function tuningDeck() {
  ...
  if (deck.cards.length > 0) {
    ...
    if (deck.cards.length < analysisAverages.averageQtd) {
      await autoGenerateDeck();
    }

    if (innexistentCategories.length > 0) {
      addCardFromSpecifiedCategory(innexistentCategories[0], suggestionNumbers);
      removeCardFromSpecifiedCategory(maiorCategoria);
    } else if (higherCategories.length > 0 && lowerCategories.length > 0) {
      addCardFromSpecifiedCategory(lowerCategories[0], suggestionNumbers);
      removeCardFromSpecifiedCategory(maiorCategoria);
    }
...
function removeCardFromSpecifiedCategory(category) {
  ...
  cardList.forEach((card) => {
    let ocorrencias = getOccurrencesInDecks(card.number, decks);
