...
async function updateAnalysisFromDeck() {
  ...
  if (deck.cards.length > 0) {
    analysisAverages = await analyzeDecks(decks);
...
async function tuningDeck() {
  let oldDeck = deck;
  let oldCardsFromDeck = cardsFromDeck;

  if (deck.cards.length > 0) {
    console.log(oldDeck);
    console.log(infoFromDeck);
    ...
    if (innexistentCategories.length > 0) {
      ...
      cardList = cardList.filter(card => {
        // Verifica se a propriedade 'categories' existe e se está definida
        if (!card.categories) return false;
    
        // Converte a string de categorias separadas por ";" em um array
        const cardCategories = card.categories.split(';').map(cat => cat.trim());
    
        // Verifica se alguma das categorias do card está em innexistentCategories
        return cardCategories.some(cat => innexistentCategories.includes(cat));
      });
...
function addCardFromSpecifiedCategory(){}

// REMOVA DE UM DECK A PIOR CARTA DE UMA CATEGORIA EM ESPECÍFICO
function removeCardFromSpecifiedCategory(){}
