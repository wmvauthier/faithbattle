  ...
  if (deck.cards.length > 0) {
    ...
    await Promise.all(
      deck.cards.map(async (card) => {
        const similarCards = await getRelatedCardsInDecks(card, decks, true, selectedStyle, selectedArchetype);
...
async function generateCostSuggestions(rerun) {
  ...
  const buttonContainer = document.getElementById("addByCostsList");
  if (!buttonContainer) {
    return;
  }
